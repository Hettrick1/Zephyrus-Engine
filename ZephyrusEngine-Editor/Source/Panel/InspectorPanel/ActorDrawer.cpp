...
void ActorDrawer::SetTransform(Zephyrus::ActorComponent::Actor* pActor, const std::string& label, const Vector3D& initialValue, std::function<void(const Vector3D&)> realTimeSetter, std::function<void(Zephyrus::ActorComponent::Actor*, const Vector3D&, const Vector3D&)> eventSetter)
{
	float transform[3] = {
	...
	if (ImGui::DragFloat3(("##" + label).c_str(), transform, 0.1f, 0.0f, 0.0f, "%.3f"))
	{
